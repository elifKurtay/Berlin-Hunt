...
public class CalculateRealDistance : MonoBehaviour
{
    ...
    public void Calc(float lat1, float lon1, float lat2, float lon2)
    {
        ...
        distanceTextObject.text = "Distance to Next Treasure: " + distance;
    ...
    void Update()
    {
        ...
        if (distance < 3f)
        {
            distanceTextObject.text = "PLEASE GO TO NEXT CHECKPOINT";
            if (audioPlay)
            {
                gotCheckpoint = true;
                audio.Pause();
        ...
        {
            distanceTextObject.text = "LESS THAN 6 METERS";
            if (!audioPlay && !gotCheckpoint)
            {
                audio.Play();
                audioPlay = true;
